...
export class AIClientService {
  ...
  async getETAFromAI(
    ...
    try {
      ...
      this.logger.error(`AI ETA failed: ${message}, using fallback`);
      // Fallback: tính ETA thủ công
  ...
  async getSurgeFromAI(
    ...
    try {
      ...
      const message = error instanceof Error ? error.message : String(error);
      this.logger.error(`AI Surge failed: ${message}, using fallback`);
      // Fallback: tính surge thủ công
